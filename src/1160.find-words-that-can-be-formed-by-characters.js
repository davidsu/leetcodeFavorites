...
const countCharacters = function (words, chars) {
  ...
  const charsFreq = freq(chars)
  const isValid = word => {
    const _freq = freq(word)
    return Object.entries(_freq).every(([char, count]) => (charsFreq[char] || 0) >= count)
  }
  return words.filter(isValid).join('').length
}
