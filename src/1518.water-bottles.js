 ...
 * @return {number}
 */
const numWaterBottles = function (numBottles, numExchange, emptyBottles = 0) {
  if (numBottles === 0) return 0
  const newEmpty = numBottles + emptyBottles
  const newFull = Math.floor(newEmpty / numExchange)
  const nextEmpty = newEmpty - newFull * numExchange
  return numBottles + numWaterBottles(newFull, numExchange, nextEmpty)
}
