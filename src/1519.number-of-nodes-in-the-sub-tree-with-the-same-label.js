 ...
 * @return {number[]}
 */
function mergeMap(target, other) {
  for (const [c, count] of Object.entries(other)) {
    target[c] = (target[c] || 0) + count
  }
}
function visit(node, labels) {
  node.map = {}
  const q = [[node, undefined]]
  let i = 0
  while (i < q.length) {
    const [curr, parent] = q[i]
  ...
  while (q.length) {
    ...
    node.map[labels[node.id]] = (node.map[labels[node.id]] || 0) + 1
    node.count = node.map[labels[node.id]]
    if (parent) mergeMap(parent.map, node.map)
  }
}
const countSubTrees = function (n, edges, labels) {
  const g = new Array(n)
  for (const [parent, child] of edges) {
    g[parent] = g[parent] || { neighbors: [], id: parent }
    g[child] = g[child] || { neighbors: [], id: child }
    g[parent].neighbors.push(g[child])
    g[child].neighbors.push(g[parent])
  }
