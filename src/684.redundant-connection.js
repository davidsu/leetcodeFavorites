/*
 * @lc app=leetcode id=684 lang=javascript
 *
 * [684] Redundant Connection
 *
 * https://leetcode.com/problems/redundant-connection/description/
 *
 * algorithms
 * Medium (57.02%)
 * Total Accepted:    93.6K
 * Total Submissions: 164.1K
 * Testcase Example:  '[[1,2],[1,3],[2,3]]'
 *
 *
 * In this problem, a tree is an undirected graph that is connected and has no
 * cycles.
 *
 * The given input is a graph that started as a tree with N nodes (with
 * distinct values 1, 2, ..., N), with one additional edge added.  The added
 * edge has two different vertices chosen from 1 to N, and was not an edge that
 * already existed.
 *
 * The resulting graph is given as a 2D-array of edges.  Each element of edges
 * is a pair [u, v] with u < v, that represents an undirected edge connecting
 * nodes u and v.
 *
 * Return an edge that can be removed so that the resulting graph is a tree of
 * N nodes.  If there are multiple answers, return the answer that occurs last
 * in the given 2D-array.  The answer edge [u, v] should be in the same format,
 * with u < v.
 * Example 1:
 *
 * Input: [[1,2], [1,3], [2,3]]
 * Output: [2,3]
 * Explanation: The given undirected graph will be like this:
 * ⁠ 1
 * ⁠/ \
 * 2 - 3
 *
 *
 * Example 2:
 *
 * Input: [[1,2], [2,3], [3,4], [1,4], [1,5]]
 * Output: [1,4]
 * Explanation: The given undirected graph will be like this:
 * 5 - 1 - 2
 * ⁠   |   |
 * ⁠   4 - 3
 *
 *
 * Note:
 * The size of the input 2D-array will be between 3 and 1000.
 * Every integer represented in the 2D-array will be between 1 and N, where N
 * is the size of the input array.
 *
 *
 *
 *
 *
 * Update (2017-09-26):
 * We have overhauled the problem description + test cases and specified
 * clearly the graph is an undirected graph. For the directed graph follow up
 * please see Redundant Connection II). We apologize for any inconvenience
 * caused.
 *
 */
/**
 * @param {number[][]} edges
 * @return {number[]}
 */
function findLoop(graph, node, from, visiting = new Set(), path = []) {
  if (visiting.has(node)) {
    path.splice(0, path.indexOf(node))
    path.push(node)
    return path
  }
  path.push(node)
  visiting.add(node)
  for (const n of graph[node] || []) {
    if (n !== from) {
      const result = findLoop(graph, n, node, visiting, path)
      if (result) return result
    }
  }
  path.pop()
}
function lastEdge(loop, edges) {
  const edgesInLoop = new Set()
  for (let i = 1; i < loop.length; i++) {
    edgesInLoop.add(`${loop[i]},${loop[i - 1]}`)
    edgesInLoop.add(`${loop[i - 1]},${loop[i]}`)
  }
  for (let i = edges.length - 1; i >= 0; i--) {
    if (edgesInLoop.has(edges[i].join(','))) return edges[i]
  }
}
const _findRedundantConnection = function (edges) {
  let N = 0
  const graph = edges.reduce((g, [u, v]) => {
    g[u] = g[u] || []
    g[v] = g[v] || []
    g[u].push(v)
    g[v].push(u)
    N = Math.max(N, u, v)
    return g
  }, {})
  for (let i = 0; i <= N; i++) {
    const loop = findLoop(graph, i)
    if (loop) {
      return lastEdge(loop, edges)
    }
  }
}

function find(union, v) {
  while (union[v] !== v) {
    union[v] = union[union[v]]
    v = union[v]
  }
  return v
}
function connect(union, u, v) {
  union[find(union, v)] = union[find(union, u)]
}
const findRedundantConnection = edges => {
  const union = Array.from({ length: edges.length + 1 }, (_, i) => i)
  for (const [u, v] of edges) {
    if (find(union, v) === find(union, u)) return [u, v]
    connect(union, u, v)
  }
}

module.exports = {
  findRedundantConnection,
  _findRedundantConnection
}
